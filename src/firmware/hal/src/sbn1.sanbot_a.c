...
void sbn1HandleReceived ( void )
{
	...
	switch ( _op )
	{
		...
		case SBN1_OP_MOVE_REQUEST:
		{
			...
			unsigned char * medium = ( unsigned char * ) & ( temp_params.angle );

			for ( i = 0; i < 4; i++ )
			{
				medium[i] = ( unsigned char ) nRF_ReceiveBuffer[0x02 + i];
			}

			temp_params.velocity = (nRF_ReceiveBuffer[0x06] << 8) + nRF_ReceiveBuffer[0x07];
			temp_params.spin     = (nRF_ReceiveBuffer[0x08] << 8) + nRF_ReceiveBuffer[0x09];
			temp_params.duration = (nRF_ReceiveBuffer[0x0A] << 8) + nRF_ReceiveBuffer[0x0B];
